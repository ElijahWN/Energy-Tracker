...
/**
 * Provides server-side utility functions for various calculations and data manipulations,
...
export default class UtilsController {
     ...
     * @returns {Number}
     */
    static getLocationPower(appliances) {
        if (!Array.isArray(appliances)) return 0;
        return appliances.reduce(
            (sum, app) => sum + (app ? this.getAppliancePower(app) : 0),
            0
        );
    }
     ...
     * @returns {number} Power usage in watt-hours, or 0 if invalid.
     */
    static getAppliancePower(appliance) {
        if (!appliance || !appliance.type || !appliance.type.name) {
            return 0;
        }
        const appType = ApplianceTypeModel.types.find(
